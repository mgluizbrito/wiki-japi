<x-html>
    ...
    <section id="posts" style="margin-top: 80px;" class="container-fluid">
        <div class="post-ops mb-4 pt-3 d-flex justify-content-center align-items-center">
            <h1 class="color-black" style="font-size: 32px">Minhas Identificações:</h1>
        </div>
        ...
        <div class="observations d-flex flex-wrap gap-4">
            @foreach($observations as $obs)
                <x-observation_card :data="[
                    ...
                    'imgPath' => $obs->observation->photo_url,
                    'desc' => $obs->observation->desc,
                    'datetime' => \Carbon\Carbon::parse($obs->observation->datetime)->format('d/m/Y'). ' às ' . \Carbon\Carbon::parse($obs->datetime)->format('H:i'),
                    'name' => $obs->observation->user->name,

                    'sci_name' => $obs->observation->ident->scientific_name ?? null,
                    'com_name' => $obs->observation->ident->common_name ?? null,
                ]" />
            @endforeach
        </div>
    </section>
